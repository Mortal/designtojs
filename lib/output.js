...
Output.prototype.compileOptionalArgs = function(args){
    ...
    if(i<args.length){
        ...
        for(var i2=i; i2<args.length; i2++){
            var current = args[i2];
            if(current.length > 1){
                current.splice(1, current.length-1);  //Remove the ?
            }
...
Output.prototype.extendHead = function(){
    ...
    var headEnd = this.find(['end_head'], this.consumed+1, {
        skip: ['space', 'comment', 'word', 'operator', 'colon', 'literal', 'comma']
    });
    if(headEnd === -1) return;
    ...
    if(funcword !== -1){
        var wordType = this.tokens[funcword][1];
        if(wordType === 'while' || wordType === 'for'){
            return;
        }
    }
    ...
    var startType;
    if(funcstart !== -1){
        startType = this.tokens[funcstart][0]; 
    } else {
        funcstart = this.consumed -1;
    }

    if(!startType || startType !== 'function'){
