...
Output.prototype.compileColon = function(){
    ...
    var nextFunction = this.find(['function'], this.consumed, {
        skip: ['space', 'comment'],
    });
...
Output.prototype.extendHead = function(){
    ...
    for(var i=this.consumed+1; i<tokens.length; i++){
        ...
        if(type === 'conditional' || type === 'operator' || type === 'colon'){
            mustBeComma = true;
            continue;
        }
        if(type !== 'literal' && type !== 'word') return;
    }
    var headEnd = i;
